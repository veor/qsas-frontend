...
@Component({
  selector: 'app-one-poor-fam-ranking',
  standalone: true,
  imports: [CommonModule, FormsModule, MatIconModule],
  templateUrl: './one-poor-fam-ranking.component.html',
  styleUrl: './one-poor-fam-ranking.component.css'
})
export class OnePoorFamRankingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.adminService.getTopByOnePoorFam().subscribe({
      next: (data) => {
  ...
  confirmRejections(): void {
    ...
    this.adminService.rejectOnePoorFamApplicants(this.rejectedList).subscribe({
      next: (res) => {
  ...
  exportToExcel(): void {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'One Poor Fam Ranking');

    const timestamp = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(workbook, `one-poor-fam-ranking-${timestamp}.xlsx`);
  }
exportAllToExcel(): void {
  this.adminService.exportOnePoorFamAll().subscribe({
    next: (data) => {
      ...
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, 'One Poor Fam All');

      const timestamp = new Date().toISOString().slice(0, 10);
      XLSX.writeFile(workbook, `one-poor-fam-all-${timestamp}.xlsx`);
    },
    error: () => {
